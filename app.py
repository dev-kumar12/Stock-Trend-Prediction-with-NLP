...
def get_stock_data(ticker):
    # download 1 year of data for enough history
...
def get_finbert_sentiment(text_list, tokenizer, model):
    if not text_list:  # empty list safeguard
        return []
...
if st.button("Predict Tomorrow's Trend"):
    with st.spinner("Running prediction pipeline..."):
        ...
        if not news_df.empty and 'title' in news_df.columns:
            ...
            sentiments = get_finbert_sentiment(news_df['cleaned_title'].tolist(),
            ...
            if pd.isna(today_sentiment):
                today_sentiment = 0.0
        else:
            st.warning("No news articles with titles found. Defaulting sentiment to neutral.")
            today_sentiment = 0.0
        ...
        col2.metric(label="Predicted Price for Tomorrow",
                    value=f"₹{prediction_actual:.2f}",
                    delta=f"₹{prediction_actual - last_close_price:.2f}")
